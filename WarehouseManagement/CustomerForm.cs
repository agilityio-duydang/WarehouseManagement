    ...
    public partial class CustomerForm : BaseForm
    {
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                sfNPL.FileName = "Danh sách lịch sử bán trả hàng_" + DateTime.Today.ToString("dd/MM/yyyy").Replace("/", "_") + ".xls";
        ...
        private void btnExportDept_Click(object sender, EventArgs e)
        {
            ...
            {
                SaveFileDialog sfNPL = new SaveFileDialog();
                sfNPL.FileName = "Danh sách nợ cần thu từ khách_" + DateTime.Today.ToString("dd/MM/yyyy").Replace("/", "_") + ".xls";
                sfNPL.Filter = "Excel files| *.xls";
                if (ShowMessage("Bạn có muốn xuất thông tin này ra File Excel không? ", true, false) == "Yes")
                {
                    if (sfNPL.ShowDialog(this) == DialogResult.OK && sfNPL.FileName != "")
        ...
        private void btnExportReceipts_Click(object sender, EventArgs e)
        {
            ...
            {
                SaveFileDialog sfNPL = new SaveFileDialog();
                sfNPL.FileName = "Danh sách lịch sử thu từ khách_" + DateTime.Today.ToString("dd/MM/yyyy").Replace("/", "_") + ".xls";
                sfNPL.Filter = "Excel files| *.xls";
                if (ShowMessage("Bạn có muốn xuất thông tin này ra File Excel không? ", true, false) == "Yes")
                {
                    if (sfNPL.ShowDialog(this) == DialogResult.OK && sfNPL.FileName != "")
                    {
                        ...
                        Janus.Windows.GridEX.Export.GridEXExporter gridEXExporter1 = new Janus.Windows.GridEX.Export.GridEXExporter();
                        gridEXExporter1.GridEX = dgListReceipts;
                        try
                        {
                            System.IO.Stream str = sfNPL.OpenFile();
                            gridEXExporter1.Export(str);
